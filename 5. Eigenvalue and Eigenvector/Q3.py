...
print("x axis: \t", acos(normal[0, 2] / np.linalg.norm(normal[:, 2]))/180*np.pi)
print("y axis: \t", acos(normal[1, 2] / np.linalg.norm(normal[:, 2]))/180*np.pi)
print("z axis: \t", acos(normal[2, 2] / np.linalg.norm(normal[:, 2]))/180*np.pi)

print("The angles that the normal to the principal plane which has the minimum principal stress makes with the:")
print("x axis: \t", acos(normal[0, 0] / np.linalg.norm(normal[:, 0]))/180*np.pi)
print("y axis: \t", acos(normal[1, 0] / np.linalg.norm(normal[:, 0]))/180*np.pi)
print("z axis: \t", acos(normal[2, 0] / np.linalg.norm(normal[:, 0]))/180*np.pi)