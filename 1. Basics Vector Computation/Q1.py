...
angleX = np.arccos(np.dot(AcrossB, [1, 0, 0]) / (np.linalg.norm(AcrossB) * np.linalg.norm([1, 0, 0]))) * 180 / np.pi
angleY = np.arccos(np.dot(AcrossB, [0, 1, 0]) / (np.linalg.norm(AcrossB) * np.linalg.norm([0, 1, 0]))) * 180 / np.pi
angleZ = np.arccos(np.dot(AcrossB, [0, 0, 1]) / (np.linalg.norm(AcrossB) * np.linalg.norm([0, 0, 1]))) * 180 / np.pi
...
BcrossDunit = np.array([BcrossD[0] / np.linalg.norm(BcrossD), BcrossD[1] / np.linalg.norm(BcrossD), BcrossD[2] / np.linalg.norm(BcrossD)])
...
if magD > magMax:
  nameMax = "D"
  magMax = magD
print("\tThe vector with the largest magnitude is ", nameMax, ". Its magnitude is ", magMax, ".")